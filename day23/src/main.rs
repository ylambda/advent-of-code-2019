...
fn main() {
    ...
    let mut p_inputs: HashMap<usize, Vec<isize>> = HashMap::new();
    let mut p_exit_codes: HashMap<usize, Result<isize, isize>> = HashMap::new();

    let mut nat_outputs: Vec<isize> = vec![-1; 2];
    let mut nat_sleeping: Vec<bool> = vec![false; amps];

    for i in 0..amps {
        programs.insert(i, Program::new(source[0], 100));
        p_outputs.insert(i, Vec::new());
    ...
    loop {
        ...
        let mut a_program = programs.get_mut(&amp).unwrap();
        let mut outputs = p_outputs.get_mut(&amp).unwrap();
        let mut exit_code = p_exit_codes.get_mut(&amp).unwrap();

        while exit_code.is_ok() {

            //println!("Amplifier {}: Running", amp);

            {
                let input = p_inputs.get(&amp).unwrap();
                *exit_code = run_program(&mut a_program, &input);
            }
            ...
            outputs.push(exit_code.unwrap());
            if outputs.len() % 3 == 0 {
                let y = outputs.pop().unwrap();
                let x = outputs.pop().unwrap();
                let n = outputs.pop().unwrap() as usize;
        ...
        if amp == 0 {
            let all_sleeping = nat_sleeping.iter().fold(true, |acc, &x| acc && x);
            if all_sleeping {
                println!("NAT 255: All Sleeping - Sending output ({}, {})", nat_outputs[0], nat_outputs[1]);
                let mut input = p_inputs.get_mut(&0).unwrap();
                input.push(nat_outputs[0]);
                input.push(nat_outputs[1]);
                nat_sleeping[0] = false;
            }
        }
    }
}
