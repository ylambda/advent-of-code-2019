...
fn main() {
    ...
    loop {
        ...
        while output.is_ok() {
            ...
            match output {
                Ok(0) => { grid.insert(attempt_pos, '#'); },
                Ok(1) => { grid.insert(attempt_pos, '.'); pos = attempt_pos; },
                Ok(2) => { grid.insert(attempt_pos, 'o'); pos = attempt_pos; }
        ...
        if output == Err(3) {
            ...
            while direction.is_none() {
                let mut possible_direction = match orientation {
                ...
                let tile = grid.get(&move_to(pos, possible_direction as isize));

                if tile.is_some() && *tile.unwrap() == '#' {
                    possible_direction = orientation;
                } else {
                    orientation = possible_direction;
                    println!("Moving to {:?}. Looking {:?}", possible_direction, orientation);
                    direction = Some(possible_direction);
                    continue;
                }
                ...
                if tile.is_some() && *tile.unwrap() == '#' {
                    ...
                    println!("Skipping {:?}. Looking {:?}", possible_direction, orientation);
                    continue;
                } else {
                    orientation = possible_direction;
                    println!("Moving to {:?}. Looking {:?}", possible_direction, orientation);
            ...
            attempt_pos = move_to(pos, direction.unwrap() as isize);
            input.push(direction.unwrap() as isize);
            output = Ok(-1);
        }
...
fn move_to(pos: (isize, isize), direction: isize) -> (isize, isize) {
...
fn paint_grid(grid: &HashMap<(isize, isize), char>, pos: (isize, isize)) {
    // find dimensions
    let mut min_x = 0;
    let mut min_y = 0;
    let mut max_x = 0;
